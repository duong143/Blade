...
<div class="card">
  <div class="card-header">
    <h3 class="card-title">Sửa combo #{{ $combo->id }}</h3>
  </div>
    ...
    <form method="POST" action="{{ route('admin.combos.update', $combo->id) }}" enctype="multipart/form-data">
      ...
      <div class="row">
        <div class="col-md-4 mb-3">
          ...
          <input name="code" class="form-control" value="{{ old('code', $combo->code) }}">
        </div>

        <div class="col-md-8 mb-3">
          <label class="form-label">Tên combo</label>
          <input name="title" class="form-control" value="{{ old('title', $combo->title) }}" required>
        </div>
      </div>
      ...
      <div class="mb-2">
        ...
        <img src="{{ asset('storage/' . $combo->image) }}" style="height:80px;width:160px;object-fit:cover;border-radius:6px;">
      </div>
      @endif

      <div class="mb-3">
        <label class="form-label">Đổi ảnh đại diện (tuỳ chọn)</label>
      ...
      <div class="mb-3">
        <label class="form-label">Thêm ảnh slider đầu trang</label>
        <input type="file" name="gallery_images[]" class="form-control" accept="image/*" multiple>
        <small class="text-muted">Có thể chọn nhiều ảnh để thêm vào banner đầu trang chi tiết combo</small>
      </div>

      @if($combo->images->isNotEmpty())
      <div class="mb-3">
        <label class="form-label d-block">Danh sách ảnh slider hiện tại</label>
        ...
        <div class="d-flex flex-wrap gap-3">
          ...
          <div style="width:170px;">
            ...
            <button
              ...
              data-url="{{ route('admin.combos.images.destroy', $img->id) }}">
              Xóa ảnh này
            </button>
          </div>
          @endforeach
      ...
      <div class="row">
        <div class="col-md-6 mb-3">
          ...
          <input name="from_location" class="form-control" value="{{ old('from_location', $combo->from_location) }}">
        </div>
        <div class="col-md-6 mb-3">
          <label class="form-label">Điểm đến</label>
          <input name="to_location" class="form-control" value="{{ old('to_location', $combo->to_location) }}">
        </div>
      </div>
      ...
      <div class="row">
        <div class="col-md-4 mb-3">
          ...
          <input type="number" name="duration_days" class="form-control" value="{{ old('duration_days', $combo->duration_days) }}" min="1">
        </div>
        <div class="col-md-4 mb-3">
          <label class="form-label">Số đêm</label>
          <input type="number" name="duration_nights" class="form-control" value="{{ old('duration_nights', $combo->duration_nights) }}" min="0">
        </div>
        <div class="col-md-4 mb-3">
          <label class="form-label">Đặt trước (ngày)</label>
          <input type="number" name="preorder_days" class="form-control" value="{{ old('preorder_days', $combo->preorder_days) }}" min="0">
        </div>
      </div>

      <div class="mb-3">
        <label class="form-label">Mô tả ngắn</label>
        <textarea name="short_desc" class="form-control" rows="2">{{ old('short_desc', $combo->short_desc) }}</textarea>
      </div>

      <div class="mb-3">
        <label class="form-label">Mô tả chi tiết</label>
        <textarea name="description" class="form-control" rows="5">{{ old('description', $combo->description) }}</textarea>
      </div>

      <div class="mb-3">
        <label class="form-label">Lịch trình chi tiết</label>
        <textarea name="itinerary_detail" class="form-control" rows="8">{{ old('itinerary_detail', $combo->itinerary_detail) }}</textarea>
      ...
      <div class="mb-3">
        ...
        <textarea name="hotel_amenities" class="form-control" rows="6">{{ old('hotel_amenities', $combo->hotel_amenities) }}</textarea>
        <small class="text-muted">Nội dung phần hiển thị dưới tiêu đề "Tiện nghi khách sạn"</small>
      </div>

      <div class="mb-3">
        <label class="form-label">Trạng thái</label>
        <select name="status" class="form-select">
          <option value="1" @selected(old('status', $combo->status)==1)>Hiển thị</option>
          <option value="0" @selected(old('status', $combo->status)==0)>Ẩn</option>
        </select>
      </div>
...
<script>
  document.querySelectorAll('.delete-image-btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
      if (!confirm('Xóa ảnh banner này?')) return;
